...
def get_secret(keyVaultName ,secretName):
...
def call_search_api(search_service, search_api_version, resource_type, resource_name, method, credential, key_vault_name, body=None):
    """
    ...
    headers = {
        ...
        'api-key': get_secret(key_vault_name, 'azureSearchKey')
    }
...
def get_function_key(subscription_id, resource_group, function_app_name, credential, function):
    """
    ...
    # Get key
    requestUrl = f"https://management.azure.com/subscriptions/{subscription_id}/resourceGroups/{resource_group}/providers/Microsoft.Web/sites/{function_app_name}/functions/{function}/keys/mykey?api-version=2022-03-01"
    requestHeaders = {
...
def execute_setup(subscription_id, resource_group, function_app_name, search_principal_id, azure_search_use_mis, enable_managed_identities, enable_env_credentials):
    ...
    # Read required settings with fallbacks to environment variables
    key_vault_name = function_app_settings.properties.get("AZURE_KEY_VAULT_NAME") or os.environ.get("AZURE_KEY_VAULT_NAME")
    search_service = function_app_settings.properties.get("SEARCH_SERVICE") or os.environ.get("SEARCH_SERVICE") or os.environ.get("SEARCH_SERVICE_NAME")
    search_analyzer_name= function_app_settings.properties.get("SEARCH_ANALYZER_NAME") or os.environ.get("SEARCH_ANALYZER_NAME", "standard")
    ...
    search_index_interval = function_app_settings.properties.get("SEARCH_INDEX_INTERVAL") or os.environ.get("SEARCH_INDEX_INTERVAL", "PT1H")
    search_index_name = function_app_settings.properties.get("SEARCH_INDEX_NAME") or os.environ.get("SEARCH_INDEX_NAME")
    storage_container = function_app_settings.properties.get("STORAGE_CONTAINER") or os.environ.get("STORAGE_CONTAINER", "documents")
    storage_account_name = function_app_settings.properties.get("STORAGE_ACCOUNT_NAME") or os.environ.get("STORAGE_ACCOUNT_NAME")
    network_isolation = True if (function_app_settings.properties.get("NETWORK_ISOLATION") or os.environ.get("NETWORK_ISOLATION", "false")).lower() == "true" else False
    embeddings_dim = int(function_app_settings.properties.get("AZURE_EMBEDDINGS_VECTOR_SIZE") or os.environ.get("AZURE_EMBEDDINGS_VECTOR_SIZE", "3072"))
    ...
    if not search_service:
        missing.append("SEARCH_SERVICE/SEARCH_SERVICE_NAME")
    if not search_index_name:
        missing.append("SEARCH_INDEX_NAME")
    ...
    logging.info(f"Embeddings vector dimension: {embeddings_dim}")

    
    ###########################################################################
    # Get function key to be used later when creating the skillset
    ########################################################################### 
    function_key = get_function_key(subscription_id, resource_group, function_app_name, credential, "document_chunking")
    ...
    call_search_api(search_service, search_api_version, "datasources", f"{search_index_name}-datasource", "put", credential, key_vault_name, body)
    response_time = time.time() - start_time
    ...
    call_search_api(search_service, search_api_version, "indexes", f"{search_index_name}", "put", credential, key_vault_name, body)
    response_time = time.time() - start_time
            ...
            { 
                ...
                "inputs":[ 
                    ...
                    },
                    {
                        ...
                        "source":"/document/Analyzed_content"
                    },                    
                    { 
                        "name":"documentSasToken",
                        "source":"/document/metadata_storage_sas_token"
            ...
            "selectors": [
                {
                    ...
                    "mappings": [
                        ...
                        {
                            "name": "chunk_headers",
                            "source": "/document/chunks/*/headers",
                            "inputs": []
                        },
                        {
                            "name": "contentVector",
                        ...
                        {
                            "name":"document_level",
                            "source":"/document/document_level",
                            "inputs": []                        
                        },
                        {
                            "name":"language",
                        ...
                        {
                            "name":"chunk_type",
                            "source":"/document/chunks/*/chunk_type",
                            "inputs": []
                        },
                        {
                            "name":"jsonPath",
                            "source":"/document/chunks/*/jsonPath",
                            "inputs": []
                        },
                        {
                            "name":"keys",
                            "source":"/document/chunks/*/keys",
                            "inputs": []
                        },
                        {
                            "name":"tableIds",
                            "source":"/document/chunks/*/tableIds",
                            "inputs": []
                        },
                        {
                            "name":"tableHeaderHierarchies",
                            "source":"/document/chunks/*/tableHeaderHierarchies",
                            "inputs": []
                        },
                        {
                            "name":"qualityConfidence",
                            "source":"/document/chunks/*/qualityConfidence",
                            "inputs": []
                        },
                        {
                            "name":"ocrEngine",
                            "source":"/document/chunks/*/ocrEngine",
                            "inputs": []
                        }
                    ]
                }
    ...
    table_extraction_key = get_function_key(subscription_id, resource_group, function_app_name, credential, "table_extraction")
    if table_extraction_key is None:
            logging.error(f"Could not get function key. Please make sure the function {function_app_name}/table_extraction is deployed before running this script.")
            exit(1)

    table_analysis_key = get_function_key(subscription_id, resource_group, function_app_name, credential, "table_analysis")
    if table_analysis_key is None:
            logging.error(f"Could not get function key. Please make sure the function {function_app_name}/table_analysis is deployed before running this script.")
            exit(1)

    if azure_search_use_mis:
        body['skills'][0]['uri'] = f"{function_endpoint}/api/table-extraction"
        body['skills'][0]['authResourceId'] = f"api://{search_principal_id}"
        body['skills'][1]['uri'] = f"{function_endpoint}/api/document-chunking"
        body['skills'][1]['authResourceId'] = f"api://{search_principal_id}"
    else:
        body['skills'][0]['uri'] = f"{function_endpoint}/api/table-extraction?code={table_extraction_key}"
        body['skills'][1]['uri'] = f"{function_endpoint}/api/document-chunking?code={function_key}"

    # first delete to enforce web api skillset to be updated
    call_search_api(search_service, search_api_version, "skillsets", f"{search_index_name}-skillset-chunking", "delete", credential, key_vault_name)        

    call_search_api(search_service, search_api_version, "skillsets", f"{search_index_name}-skillset-chunking", "put", credential, key_vault_name, body)
    ...
    call_search_api(search_service, search_api_version, "indexers", f"{search_index_name}-indexer-chunk-documents", "put", credential, key_vault_name, body)
