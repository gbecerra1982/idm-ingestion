...
class DocumentIntelligenceClient:
    ...
    def __init__(self, document_filename=""):
        ...
        self.use_foundry = os.getenv('ENABLE_FOUNDRY', 'false').lower() == 'true'
        
        # ai service resource name (Azure DI); not needed when using Foundry
        self.service_name = os.environ.get('AZURE_FORMREC_SERVICE', '')
        
        # API configuration (Azure DI); bypassed when using Foundry
        ...
        if self.docint_40_api or self.use_foundry:
    ...
    def analyze_document(self, file_url, model='prebuilt-layout'):
        ...
        try:
            ...
            error_message = f"Blob client error when reading from blob storage. {e}"
        ...
        if self.use_foundry:
            try:
                from .foundry import FoundryVisionClient
                vision = FoundryVisionClient()
                result, v_errors = vision.analyze_document(data)
                errors.extend(v_errors)
                return result, errors
            except Exception as e:
                errors.append(f"Foundry analyze_document error: {e}")
                return result, errors
