...
class AzureOpenAIClient:
    ...
    def __init__(self, document_filename=""):
        ...
        if not self.use_foundry:
            ...
            from .foundry import FoundryEmbeddingsClient, FoundryChatClient
            self._foundry_embeddings = FoundryEmbeddingsClient()
            self._foundry_chat = FoundryChatClient()
    ...
    def get_completion(self, prompt, max_tokens=800, retry_after=True):
        ...
        if self.use_foundry:
            try:
                return self._foundry_chat.get_completion(prompt, max_tokens=max_tokens)
            except Exception as e:
                logging.error(f"[aoai]{self.document_filename} foundry get_completion: {e}")
                raise
    ...
    def get_embeddings(self, text, retry_after=True):
        ...
        if self.use_foundry:
            try:
                return self._foundry_embeddings.get_embeddings(text)
            except Exception as e:
                logging.error(f"[aoai]{self.document_filename} foundry get_embeddings: {e}")
                raise
