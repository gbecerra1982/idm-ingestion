...
class BaseChunker:
    ...
    def _create_chunk(self, chunk_id, content, summary="", embedding_text="", title="", page=0, offset=0, related_images=[], related_files=[], headers=[], chunk_type="text", table_ids=None, quality_confidence=None, ocr_engine=""):
        """
        ...
        if table_ids:
            chunk["tableIds"] = table_ids
        if quality_confidence is not None:
            chunk["qualityConfidence"] = quality_confidence
        if ocr_engine:
            chunk["ocrEngine"] = ocr_engine
