...
class DocumentChunker:
    ...
    def chunk_document(self, data):
        ...
        try:
            ...
            try:
                # Some chunkers expect DI document payload; others take no args
                chunks = chunker.get_chunks(data)
            except TypeError:
                chunks = chunker.get_chunks()
            try:
                # Optional direct indexing to Azure AI Search
                if os.getenv('ENABLE_DIRECT_INDEXING', 'false').lower() == 'true':
                    from publisher.search_publisher import SearchPublisher
                    logging.info("[document_chunking] Direct indexing enabled. Publishing chunks to AI Search…")
                    SearchPublisher().publish_chunks(chunks)
            except Exception as e:
                logging.error(f"[document_chunking] Direct indexing failed: {e}")
        except Exception as e:
    ...
    def extract_tables(self, data):
        """Extract tables of the document."""
        content = ""
        tables = []
        errors = []
        warnings = []
        ...
        if extension.lower() != 'pdf':
            return content, tables, errors, warnings
        
        try:
            chunker = ChunkerFactory().get_chunker(extension, data)
            content, tables = chunker.get_tables()
        except Exception as e:
            errors.append(self._error_message(exception=e, filename=filename))

        elapsed_time = time.time() - start_time
        logging.info(
            f"[table-extraction][{filename}] Finished table extraction in {elapsed_time:.2f} seconds. "
            f"{len(tables)} tables. {len(errors)} errors. {len(warnings)} warnings."
        )
        return content, tables, errors, warnings
    

    def analyze_table(self, data):
        """Analyze a table of the document."""
        description = ""
        errors = []
        warnings = []
        start_time = time.time()
        ...
        if extension != 'pdf':
            return description, errors, warnings
        
        try:
            chunker = ChunkerFactory().get_chunker(extension, data)
            description = chunker.generate_table_desc(data)
        except Exception as e:
            errors.append(self._error_message(exception=e, filename=filename))

        elapsed_time = time.time() - start_time
        logging.info(
            f"[table-analysis][{filename}] Finished table analysis in {elapsed_time:.2f} seconds. "
            f"{len(description)} description. {len(errors)} errors. {len(warnings)} warnings."
        )
        return description, errors, warnings

    def understand_tables(self, data):
        """Full table understanding pipeline: OCR + CU + artifact URLs."""
        content = ""
        tables = []
        errors = []
        warnings = []
        ...
        try:
            chunker = ChunkerFactory().get_chunker(extension, data)
            content, tables = chunker.understand_tables()
        except Exception as e:
            errors.append(self._error_message(exception=e, filename=filename))

        elapsed_time = time.time() - start_time
        logging.info(
            f"[table-understanding][{filename}] Finished table understanding in {elapsed_time:.2f} seconds. "
            f"{len(tables)} tables. {len(errors)} errors. {len(warnings)} warnings."
        )
        return content, tables, errors, warnings
