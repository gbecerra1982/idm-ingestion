...
@app.route(route="table-extraction", auth_level=func.AuthLevel.FUNCTION)
def table_extraction(req: func.HttpRequest) -> func.HttpResponse:
    try:
        body = req.get_json()
        jsonschema.validate(body, schema=_get_request_schema())
        ...
        if body:
            # Log the incoming request
            logging.info(f'[table-extraction] Invoked table-extraction skill. Number of items: {len(body["values"])}.')
            for i, item in enumerate(body["values"]):
                data = item["data"]
                logging.info(f'[table-extraction] Item {i + 1}: File {data["documentUrl"].split("/")[-1]}, Content Type {data["documentContentType"]}.')
            start_time = time.time()
            # Chunk the documents
            result = _extract_tables(body)
            end_time = time.time()
            elapsed_time = end_time - start_time
            logging.info(f'[table-extraction] Finished table-extraction skill in {elapsed_time:.2f} seconds.')
            return func.HttpResponse(result, mimetype="application/json")
        else:
            error_message = "Invalid body."
            logging.error(f"[table-extraction] {error_message}")
            return func.HttpResponse(error_message, status_code=400)
    except ValueError as e:
        error_message = "Invalid body: {0}".format(e)
        logging.error(f"[table-extraction] {error_message}")
...
def _extract_tables(body):
    ...
    for value in values:
        ...
        logging.info(f"[table-extraction][{data['documentUrl'].split('/')[-1]}] extracting tables.")
        content, tables, errors, warnings = DocumentChunker().extract_tables(data)
...
@app.route(route="table-analysis", auth_level=func.AuthLevel.FUNCTION)
def table_analysis(req: func.HttpRequest) -> func.HttpResponse:
    try:
        body = req.get_json()
        jsonschema.validate(body, schema=_get_analysis_request_schema())

        if body:
            # Log the incoming request
            logging.info(f'[table-analysis] Invoked table-analysis skill. Number of items: {len(body["values"])}.')
            for i, item in enumerate(body["values"]):
                data = item["data"]
                logging.info(f'[table-analysis] Item {i + 1}: File {data["documentUrl"].split("/")[-1]}, Table Url {data["tableUrl"]}.')
            start_time = time.time()
            # Chunk the documents
            result = _analyze_table(body)
            end_time = time.time()
            elapsed_time = end_time - start_time
            logging.info(f'[table-analysis] Finished table-analysis skill in {elapsed_time:.2f} seconds.')
            return func.HttpResponse(result, mimetype="application/json")
        else:
            error_message = "Invalid body."
            logging.error(f"[table-analysis] {error_message}")
            return func.HttpResponse(error_message, status_code=400)
    except ValueError as e:
        error_message = "Invalid body: {0}".format(e)
        logging.error(f"[table-analysis] {error_message}")
...
def _analyze_table(body):
    ...
    for value in values:
        ...
        logging.info(f"[table-analysis][{data['documentUrl'].split('/')[-1]}] extracting tables.")
        description, errors, warnings = DocumentChunker().analyze_table(data)
...
def _get_analysis_request_schema():
    return {
        "$schema": "http://json-schema.org/draft-04/schema#",
        "type": "object",
        "properties": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "data": {
                            ...
                            "properties": {
                                "documentUrl": {"type": "string", "minLength": 1},
                                "tableUrl": {"type": "string", "minLength": 1},
                            },
                            "required": ["documentUrl", "tableUrl"],
                        },
                    },
                    "required": ["recordId", "data"],
                },
...
@app.route(route="table-understanding", auth_level=func.AuthLevel.FUNCTION)
def table_understanding(req: func.HttpRequest) -> func.HttpResponse:
    try:
        body = req.get_json()
        jsonschema.validate(body, schema=_get_request_schema())

        if body:
            logging.info(f'[table-understanding] Invoked table-understanding skill. Number of items: {len(body["values"])}.')
            for i, item in enumerate(body["values"]):
                data = item["data"]
                logging.info(f'[table-understanding] Item {i + 1}: File {data["documentUrl"].split("/")[-1]}, Content Type {data["documentContentType"]}.')
            start_time = time.time()
            result = _understand_tables(body)
            elapsed_time = time.time() - start_time
            logging.info(f'[table-understanding] Finished table-understanding skill in {elapsed_time:.2f} seconds.')
            return func.HttpResponse(result, mimetype="application/json")
        else:
            error_message = "Invalid body."
            logging.error(f"[table-understanding] {error_message}")
            return func.HttpResponse(error_message, status_code=400)
    except ValueError as e:
        error_message = "Invalid body: {0}".format(e)
        logging.error(f"[table-understanding] {error_message}")
...
def _understand_tables(body):
    ...
    for value in values:
        ...
        logging.info(f"[table-understanding][{data['documentUrl'].split('/')[-1]}] understanding tables.")
        content, tables, errors, warnings = DocumentChunker().understand_tables(data)
